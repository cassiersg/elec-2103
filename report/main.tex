...
The read of the accelerometer value by the Raspberry-Pi is very simple:
...
was a new event if since the last time it has read the register.
In the unlikely event that the touch events are emitted faster than they
can be read, there is a graceful degradation: on event is lost, but the
...
one of the originalites of our game is that the player can see a picture of
...
an open-source JavaScript webcam image capture.
...
At the start of each new game, the game server picks the two last uploaded images and sends it
to the client so that the texture rendering of the player pictures can be performed.
